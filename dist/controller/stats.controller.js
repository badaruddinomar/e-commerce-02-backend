...
export const getDashboardStats = async (req, res, next) => {
    try {
        ...
        const latestTransactionPromise = Order.find({})
            ...
            .limit(4)
            .sort({ createdAt: -1 });
        const [currentMonthProducts, lastMonthProducts, currentMonthUsers, lastMonthUsers, currentMonthOrders, lastMonthOrders, productsCount, usersCount, allOrders, lastSixMonthOrders, categories, maleUserCount, femaleUserCount, latestTransaction,] = await Promise.all([
        ...
        const userChangePercent = calculatePercentage(currentMonthUsers.length, lastMonthUsers.length);
        const orderChangePercent = calculatePercentage(currentMonthOrders.length, lastMonthOrders.length);
        const productChangePercent = calculatePercentage(currentMonthProducts.length, lastMonthProducts.length);
        ...
        const revenueChangePercent = calculatePercentage(currentMonthRevenue, lastMonthRevenue);
